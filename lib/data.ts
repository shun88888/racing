...
const sponsorLogo = (label: string) =>
  `data:image/svg+xml;utf8,${encodeURIComponent(
    `<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 240 80'><rect width='240' height='80' fill='%23141414' stroke='%238a8a8a' stroke-width='1'/><text x='50%' y='50%' font-family='Oswald, Arial' font-size='24' font-weight='600' fill='%23f5f5f5' text-anchor='middle' dominant-baseline='middle'>${label}</text></svg>`
  )}`;

export const SPONSORS: Sponsor[] = Array.from({ length: 14 }, (_, i) => {
  const n = String(i + 1).padStart(2, "0");
  const tiers: Sponsor["tier"][] = ["gold", "silver", "bronze", "partner"];
  return {
    id: `s${n}`,
    name: `SPONSOR ${n}`,
    logo: sponsorLogo(`SPONSOR ${n}`),
